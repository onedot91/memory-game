...
export default function App() {
  ...
  useActivity('classic', !isFinished);
  ...
  const [desktopLayout, setDesktopLayout] = useState(() => window.matchMedia('(min-width: 851px)').matches);
  const [slotPage, setSlotPage] = useState(0);
  const pageSize = desktopLayout ? 8 : Math.max(1, slots.length);
  const pageCount = Math.max(1, Math.ceil(slots.length / pageSize));
  const visiblePage = Math.min(slotPage, pageCount - 1);
  const pageStart = desktopLayout ? visiblePage * pageSize : 0;
  useEffect(() => {
    const media = window.matchMedia('(min-width: 851px)');
    const resize = () => setDesktopLayout(media.matches);
    media.addEventListener('change', resize);
    return () => media.removeEventListener('change', resize);
  }, []);
  useEffect(() => {
    if (!isEditOrderMode) setSlotPage(desktopLayout ? Math.floor(activeSlotIdx / 8) : 0);
  }, [activeSlotIdx, desktopLayout, currentCategoryData.id, isEditOrderMode]);
  const changeSlotPage = (page: number) => {
    cancelAutoAdvance();
    const nextPage = Math.max(0, Math.min(pageCount - 1, page));
    setSlotPage(nextPage);
    if (!isEditOrderMode) {
      const nextIndex = slots.findIndex((slot, index) => index >= nextPage * pageSize && index < (nextPage + 1) * pageSize && !slot.revealed);
      if (nextIndex !== -1) { setActiveSlotIdx(nextIndex); setSlotInput(''); setNoticeMessage(''); }
    }
  };
  ...
  }, [currentCategoryData, cancelAutoAdvance]);
  ...
  }, [handleResetAll, advanceToNextSlot, currentTopicCategories.length, cancelAutoAdvance]);
    ...
    <div className="classic-study min-h-screen bg-[#0b0716] text-[#e2d9f3] flex flex-col font-sans select-none overflow-x-hidden">
      ...
      <div className="classic-container relative z-10 flex-1 flex flex-col max-w-5xl w-full mx-auto p-3 sm:p-4 lg:p-6">
        ...
        <main className="classic-main flex-1 flex flex-col justify-center my-3 max-w-4xl w-full mx-auto">
                    ...
                    >
                      <div className="flex items-center gap-2 w-full">
                        ...
                        <input
                          ...
                          onKeyDown={e => { if (e.key === 'Enter' && (composingInput.current || e.nativeEvent.isComposing || e.nativeEvent.keyCode === 229 || e.repeat)) e.preventDefault(); }}
                          onChange={(e) => {
          ...
          {desktopLayout && pageCount > 1 && <nav className="classic-pagination" aria-label="문제 페이지">
            <button aria-label="이전 문제 페이지" disabled={visiblePage === 0} onClick={() => changeSlotPage(visiblePage - 1)} onDragEnter={() => isEditOrderMode && visiblePage > 0 && changeSlotPage(visiblePage - 1)}>←</button>
            <select aria-label="문제 페이지 선택" value={visiblePage} onChange={e => changeSlotPage(Number(e.target.value))}>{Array.from({ length: pageCount }, (_, page) => <option key={page} value={page}>{page + 1} / {pageCount}</option>)}</select>
