...
export default function Arena({ run, roomNumber, paused, motion }: { run: Run; roomNumber: number; paused: boolean; motion: boolean }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([import('phaser'), import('./scene')]).then(([{ default: Phaser }, { DungeonScene }]) => {
      ...
      const instance = new Phaser.Game({ type: Phaser.WEBGL, parent: container.current, width: 640, height: 360, pixelArt: true, antialias: false, backgroundColor: '#100b1c', banner: false, audio: { noAudio: true }, input: { keyboard: false, mouse: false, touch: false }, scale: { mode: Phaser.Scale.FIT, autoCenter: Phaser.Scale.CENTER_BOTH }, fps: { target: 60, forceSetTimeOut: false }, scene: [battle] });
    ...
    return () => { disposed = true; game.current?.destroy(true); game.current = null; scene.current = null; };
  }, []);
  useEffect(() => {
